...
<script>
  ...
  import {getPublicUnVerified, applyUpdate, approve, applyDel, getAppealById} from '../../service/api'
  ...
  export default {
    ...
    methods: {
      getPublicUnVerifiedPage() {
        getPublicUnVerified(this.query).then(re => {
      ...
      toApprove(val) {
        this.$router.push({path: '/adminpublic', query: {id: val.id}})
      },
      handleRemove(row){
